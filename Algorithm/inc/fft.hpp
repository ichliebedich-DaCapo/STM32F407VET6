...
public:
    ...
    //滑动窗口找极值
    void find_peaks_fundaments(uint32_t NumPeaks, uint16_t windowSize)
    {
        float32_t maxVal;
        uint16_t maxIdx;
     ...
     */
     void convert_fft_to_lvgl(const float_t* input, int32_t* output,size_t div,uint16_t offset) {
    ...
    float_t * get_fft_output_ptr() {
        return fft_outputbuf;  // 返回指针
    }
    float_t get_thd() const {
        return thd;  // 返回指针
    }
private:
