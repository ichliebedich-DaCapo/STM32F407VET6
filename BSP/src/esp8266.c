...
void ESP8266_Init(void)
{
    ...
    HAL_GPIO_WritePin(ESP8266_RST_Pin_Port, ESP8266_RST_Pin, GPIO_PIN_RESET);
    ...
    HAL_GPIO_Init(ESP8266_RST_Pin_Port, &GPIO_InitStruct);

    ESP8266_Rst();
}

//void ESP8266_USART(char *data,uint8_t Command)
//{
//	if(Command)//发送命令
//	{
//		HAL_UART_Transmit(&huart2, (uint8_t *)UartTxbuf,1,50);//发送1字节数据
//
//	}else//发送数据
//	{
//		HAL_UART_Transmit(&huart2, (uint8_t *)UartTxbuf,1,50);//发送1字节数据
        ...
        if ( * Data == 0x5c )  //'\'
        {
            switch ( *++Data )
            {
                case 'r':                                     //回车符
                    ...
                    HAL_UART_Transmit(&huart1, (uint8_t *)&TempData,1,50);//发送1字节数据
