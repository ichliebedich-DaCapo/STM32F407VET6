...
void custom_init(lv_ui *ui)
{
    ...
    lv_obj_add_flag(guider_ui.screen_img_album, LV_OBJ_FLAG_CLICKABLE);
    lv_obj_set_style_bg_img_src(guider_ui.screen_slider_1, &_icn_slider_alpha_15x15, LV_PART_KNOB);
    spectrum = spectrum_1;
    spectrum_len = sizeof(spectrum_1) / sizeof(spectrum_1[0]);
    spectrum_area = lv_obj_create(guider_ui.screen_player);
    ...
    lv_obj_add_event_cb(guider_ui.screen_player, spectrum_draw_event_cb, LV_EVENT_ALL, NULL);
}
...
void tracks_up(void)
{
	...
	lv_anim_set_var(&screen_player_anim_y, guider_ui.screen_player);
...
static void spectrum_anim_cb(void * a, int32_t v)
{
    ...
    // lv_img_set_zoom(guider_ui.screen_img_album, LV_IMG_ZOOM_NONE + spectrum[spectrum_i][0]);
}
...
void lv_demo_music_pause(void)
{
    ...
    lv_img_set_zoom(guider_ui.screen_img_album, LV_IMG_ZOOM_NONE);
    lv_timer_pause(sec_counter_timer);
...
{
    LV_IMG_DECLARE(_cover_2_alpha_105x105);
    LV_IMG_DECLARE(_cover_3_alpha_105x105);
    ...
    switch(track_id) {
    case 2:
        lv_img_set_src(img, &_cover_3_alpha_105x105);
        spectrum = spectrum_3;
        spectrum_len = sizeof(spectrum_3) / sizeof(spectrum_3[0]);
        break;
    case 1:
        lv_img_set_src(img, &_cover_2_alpha_105x105);
        spectrum = spectrum_2;
        spectrum_len = sizeof(spectrum_2) / sizeof(spectrum_2[0]);
        break;
    case 0:
        lv_img_set_src(img, &_cover_1_alpha_105x105);
        spectrum = spectrum_1;
        spectrum_len = sizeof(spectrum_1) / sizeof(spectrum_1[0]);
        break;
    }
...
static void track_load(uint32_t id)
{
    ...
    time = 0;
    spectrum_i_pause = 0;
    lv_slider_set_value(guider_ui.screen_slider_1, 0, LV_ANIM_OFF);
...
static void spectrum_draw_event_cb(lv_event_t * e)
{
    ...
    if(code == LV_EVENT_REFR_EXT_DRAW_SIZE) {
        ...
        r_in = (r_in * lv_img_get_zoom(guider_ui.screen_img_album)) >> 8;
