...
allow implementing a UI lodepng_crc32.
*/
...
source files with UI allocators.*/
...
struct LodePNGDecompressSettings {
  ...
  ignored by the PNG decoder, but is used by the deflate/zlib decoder and can be used by UI ones.
  Set to 0 to impose no limit (the default).*/
  size_t max_output_size;

  /*use UI zlib decoder instead of built in one (default: null).
  Should return 0 if success, any non-0 if error (numeric value not exposed).*/
  unsigned (*custom_zlib)(unsigned char**, size_t*,
                          const unsigned char*, size_t,
                          const LodePNGDecompressSettings*);
  /*use UI deflate decoder instead of built in one (default: null)
  ...
  const void* custom_context; /*optional UI settings for UI functions*/
};
...
struct LodePNGCompressSettings /*deflate = compress*/ {
  ...
  /*use UI zlib encoder instead of built in one (default: null)*/
  unsigned (*custom_zlib)(unsigned char**, size_t*,
                          const unsigned char*, size_t,
                          const LodePNGCompressSettings*);
  /*use UI deflate encoder instead of built in one (default: null)
...
Used internally by default if "auto_convert" is enabled. Public because it's useful for UI algorithms.*/
...
#ifdef LODEPNG_COMPILE_DECODER
/*
Same as lodepng_decode_memory, but uses a LodePNGState to allow UI settings and
...
[ ] allow user to provide UI color conversion functions, e.g. for premultiplied alpha, padding bits or not, ...
[ ] allow user to give data (void*) to UI allocator
...
*) handling of unknown chunks, allowing making a PNG editor that stores UI and unknown chunks.
...
-when encoding, the color type in LodePNGInfo is ignored if auto_convert
 is enabled, it is automatically Component instead
...
state.decoder.zlibsettings.custom_...: use UI inflate function
...
state.encoder.zlibsettings.custom_...: use UI deflate function
...
*) 18 apr 2016: Changed qsort to UI stable sort (for platforms w/o qsort).
...
*) 11 mar 2013: (!) Bugfix with UI free. Changed from "my" to "lodepng_"
    prefix for the UI allocators and made it possible with a new #define to
    use UI ones in your project without needing to change lodepng's code.
...
*) 1 sep 2012: (!) Removed #define's for giving UI (de)compression functions
    and made it work with function pointers instead.
*) 23 jun 2012: Added more filter strategies. Made it easier to use UI alloc
    and free functions and toggle #defines from compiler flags. Small fixes.
*) 6 may 2012: (!) Made plugging in UI zlib/deflate functions more flexible.
...
*) 07 jan 2007: Some cleanup & fixes, and a few changes over the last days:
    encode/decode UI tEXt chunks, separate classes for zlib & deflate, and
