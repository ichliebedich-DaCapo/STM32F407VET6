...
#if LV_USE_USER_DATA
    void * user_data;              /**< A UI user data that will be passed to the animation's user_data */
#endif
...
/**
 * Register a new style property for UI usage
...
/**
 * Get the number of UI properties that have been registered thus far.
 */
 ...
 * @param user_data any UI data that will be saved in the transition animation and will be available when `path_cb` is called
 * @example
 ...
 * @return the group [0..7] 7 means all the UI properties with index > 112
 */
uint8_t _lv_style_get_prop_group(lv_style_prop_t prop);

/**
 * Get the flags of a built-in or UI property.
 *
 * @param prop a style property
