 ...
 */
auto FlashStorage::read_isr_ready() -> void
{
    if (check_write_sign())
    {
        set_read_sign();
        ...
        /* 加载该页数据*/
        w25qxx_buffer_read(pForeBuffer, addr, addr_remaining);
        /* 加载下一页数据*/
        w25qxx_buffer_read(pBackBuffer, addr - index + page_size, page_size);
    }
}
 ...
 */
auto FlashStorage::write_isr_ready() -> void
{
 ...
 */
auto FlashStorage::read_isr() -> uint8_t
{
...
auto FlashStorage::write_isr(uint8_t byte) -> void
{
    if (index == page_size - 1)
    {
 ...
 */
auto FlashStorage::background_processing() -> void
{
    if (check_read_full_sign())
    {
        clear_read_full_sign();
        w25qxx_buffer_read(pBackBuffer, addr, page_size);
        addr += page_size;
    }
    if (check_write_full_sign())
    {
        ...
        if (check_write_incomplete_sign())
        {
            ...
            /* 进行残缺的写入页数据*/
            w25qxx_page_write(pBackBuffer, addr, addr_remaining);
            addr += addr_remaining;
        }
    }
}
