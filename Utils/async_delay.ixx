...
export namespace utils
{
    ...
    {
    public:
        async_delay() : start_tick(0), final_tick(0), delay_tick(100) {}
        explicit async_delay(uint32_t delay_tick) : start_tick(0), final_tick(0), delay_tick(delay_tick) {}
...
{
    template<uint32_t(*get_tick)()>
   void async_delay<get_tick>::reset()
    {
        start_tick = get_tick();
    ...
    template<uint32_t(*get_tick)()>
    bool async_delay<get_tick>::is_timeout()
    {
        // 非阻塞延迟
    ...
    template<uint32_t(*get_tick)()>
    void async_delay<get_tick>::set_delay_tick(uint32_t tick)
    {
        delay_tick = tick;
    }
}
